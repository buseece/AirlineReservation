        ...
        <?php
            ...
            if ($conn->connect_error) {
                ...
                if($result->num_rows >0){
                    ?>
                    <form action = "http://localhost/Airline/adminEditPilot2.php" method="post">
                    <?php

                    //Get the data
                    $row = $result->fetch_assoc();
                    ?>
                    Are you sure you want to update the following Pilot? <br/>
                    <p>Pilot ID: <input type="text" name="PilotID" value="<?php echo $row["PilotID"]?>" readonly /></p>
                    <p>Pilot name: <input type="text" name="PilotName" value="<?php echo $row["PilotName"]?>"  /></p>
                   <p><input type="submit" value="Update Pilot"  /></p>
                   </form>
                   <?php
                }else{
